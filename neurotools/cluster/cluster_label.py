"""
Neurotools automated labelling & merging of clusters based on DE genes that were used to construct the
nearest neighbour graph for clustering & UMAP construction.

Works by labelling clusters based on DE genes (user defined criteria), & then
for any cluster with no DE genes based on this criteria, gets merged with the most
similar cluster to it. Then repeat attempt to label, and so on until all clusters
labelled with top DE genes from neurotools balanced feature selection.
"""
...
def get_genesubset_de(data: sc.AnnData, data_sub: sc.AnnData,
    ...
    """ Gets DE genes for subset of the data, but adds to the full data.
    """
...
def label_clusters(data: sc.AnnData, groupby: str, de_key: str,
    ...
    """ Labels the clusters based on top DE genes.
    """
    #### Calling which genes are DE
    ...
    ##### Labelling clusters based on DE genes
    label_map = {}
    for i, cluster in enumerate(genes_df.columns):
        ...
        else:  # Non-significant number of DE genes, cluster isn't labelled.
...
def cluster_label(data: sc.AnnData, var_key: str,
    ...
    ### Now label clusters based on top DE genes according to criteria
